...
private[debug] final class DebugProxy(
  ...
  ): Future[DebugResponseMessage] = {
    workDoneProgress.trackFuture(
      "Initializing debugger",
      initialized.future,
    )
    clientAdapter = ClientConfigurationAdapter.initialize(args, sourceMapper)
    server.sendRequest[DebugResponseMessage](request).map { response =>
      initialized.trySuccess(())
      response
    }
  }

  private def handleLaunchRequest(
      request: DebugRequestMessage,
      debugMode: DebugMode,
  ): Future[DebugResponseMessage] = {
    this.debugMode = debugMode
    server.sendRequest[DebugResponseMessage](request)
  }

  private def handleDisconnectRequest(
      request: DebugRequestMessage,
  ...
  private def handleSetBreakpointRequest(
  ...
  private def handleCompletionRequest(
      request: RequestMessage,
      args: org.eclipse.lsp4j.debug.CompletionsArguments,
  ): Unit = {
  ...
  private def handleHotCodeReplace(
      request: RequestMessage
  ): Future[DebugResponseMessage] = {
  ...
  private val handleClientMessage: MessageConsumer = {
    ...
    case request @ InitializeRequest(args) =>
      handleInitializeRequest(request, args).failed.foreach { error =>
        scribe.error(s"Failed to handle initialize request", error)
      }
    case request @ LaunchRequest(debugMode) =>
      handleLaunchRequest(request, debugMode).failed.foreach { error =>
        scribe.error(s"Failed to handle launch request", error)
      }
    case request @ DisconnectRequest(args) =>
      handleDisconnectRequest(request, args).failed.foreach { error =>
        scribe.error(s"Failed to handle disconnect request", error)
      }
    case request @ SetBreakpointRequest(args) =>
      handleSetBreakpointRequest(request, args).foreach { response =>
        client.consume(DebugProtocol.syntheticResponse(request, response))
      }
    case request @ CompletionRequest(args) =>
      handleCompletionRequest(request, args)
    case request @ HotCodeReplace(_) =>
      handleHotCodeReplace(request).foreach { response =>
        client.consume(response)
      }
