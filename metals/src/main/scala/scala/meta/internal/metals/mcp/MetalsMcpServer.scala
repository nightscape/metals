  ...
  private def createDebugMainTool(): AsyncToolSpecification = {
    ...
    new AsyncToolSpecification(
      ...
      withErrorHandling { (exchange, arguments) =>
        ...
        scribe.info(s"[MCP] Extracted ${initialBreakpoints.size} initial breakpoints from MCP request")
        initialBreakpoints.foreach { bp =>
          scribe.info(s"[MCP] Breakpoint: $bp")
        }
        ...
        debugDiscovery
          ...
          .flatMap { debugParams =>
            debugProvider.startForMcp(
              debugParams,
              _ => (),
              initialBreakpoints, // Fixed - passing actual breakpoints
            )
          }
          .map { session =>
